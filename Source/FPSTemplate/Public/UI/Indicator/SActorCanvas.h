...
public:
	...
	public: 
		...
		FSlot(UIndicatorController* InIndicator)
			: TSlotBase<FSlot>()
	...
	private:
		...
		UIndicatorController* Indicator;
...
private:
	void OnIndicatorAdded(UIndicatorController* Indicator);
	void OnIndicatorRemoved(UIndicatorController* Indicator);

	void AddIndicatorForEntry(UIndicatorController* Indicator);
	void RemoveIndicatorForEntry(UIndicatorController* Indicator);

	using FScopedWidgetSlotArguments = TPanelChildren<FSlot>::FScopedWidgetSlotArguments;
	FScopedWidgetSlotArguments AddActorSlot(UIndicatorController* Indicator);
	...
	void GetOffsetAndSize(const UIndicatorController* Indicator,
		FVector2D& OutSize, 
		FVector2D& OutOffset,
...
private:
	TArray<TObjectPtr<UIndicatorController>> AllIndicators;
	TArray<UIndicatorController*> InactiveIndicators;

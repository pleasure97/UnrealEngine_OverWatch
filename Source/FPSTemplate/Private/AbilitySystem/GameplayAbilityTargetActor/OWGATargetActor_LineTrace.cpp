...
AOWGATargetActor_LineTrace::AOWGATargetActor_LineTrace()
{
}

void AOWGATargetActor_LineTrace::Configure(
	...
	bool bInUseAimingSpreadModifier,
	float InMaxRange,
	float InBaseSpread,
	float InAimingSpreadModifier,
	...
	int32 InNumberOfTraces)
{
	...
	bUseAimingSpreadModifier = bInUseAimingSpreadModifier; 
	MaxRange = InMaxRange; 
	BaseSpread = InBaseSpread; 
	AimingSpreadModifier = InAimingSpreadModifier; 
...
void AOWGATargetActor_LineTrace::DoTrace(TArray<FHitResult>& HitResults, const UWorld* World, const FGameplayTargetDataFilterHandle FilterHandle, const FVector& Start, const FVector& End, FName ProfileName, const FCollisionQueryParams Params)
{
	LineTraceWithFilter(HitResults, World, FilterHandle, Start, End, ProfileName, Params);
}

void AOWGATargetActor_LineTrace::ShowDebugTrace(TArray<FHitResult>& HitResults, EDrawDebugTrace::Type DrawDebugType, float Duration)
{
	...
	if (bDebug)
	{
		...
		{
			PrimaryPC->GetPlayerViewPoint(ViewStart, ViewRotator); 
		}
...
#if ENABLE_DRAW_DEBUG
void AOWGATargetActor_LineTrace::DrawDebugLineTraceMulti(const UWorld* World, const FVector& Start, const FVector& End, EDrawDebugTrace::Type DrawDebugType, bool bHit, const TArray<FHitResult>& OutHits, FLinearColor TraceColor, FLinearColor TraceHitColor, float DrawTime)
{

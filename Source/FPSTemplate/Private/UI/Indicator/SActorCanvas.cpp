...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	if (IndicatorComponent == nullptr)
	{
		...
		if (IndicatorComponent)
		{
			...
			for (UIndicatorController* Indicator : IndicatorComponent->GetIndicators())
			{
	...
	if (LocalPlayer)
	{
		...
		if (LocalPlayer->GetProjectionData(LocalPlayer->ViewportClient->Viewport, /*out*/ ProjectionData))
		{
			...
			for (int32 ChildIndex = 0; ChildIndex < CanvasChildren.Num(); ++ChildIndex)
			{
				...
				UIndicatorController* Indicator = CurChild.Indicator;
...
void SActorCanvas::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	if (bShowAnyIndicators)
	{
		...
		for (int32 ChildIndex = 0; ChildIndex < SortedSlots.Num(); ++ChildIndex)
		{
			...
			const UIndicatorController* Indicator = CurChild.Indicator;
...
void SActorCanvas::OnIndicatorAdded(UIndicatorController* Indicator)
{
...
void SActorCanvas::OnIndicatorRemoved(UIndicatorController* Indicator)
{
	RemoveIndicatorForEntry(Indicator);
...
void SActorCanvas::AddIndicatorForEntry(UIndicatorController* Indicator)
{
	...
	{
		TWeakObjectPtr<UIndicatorController> IndicatorPtr(Indicator);
		AsyncLoad(IndicatorClass, [this, IndicatorPtr, IndicatorClass]() {
			if (UIndicatorController* Indicator = IndicatorPtr.Get())
			{
...
void SActorCanvas::RemoveIndicatorForEntry(UIndicatorController* Indicator)
{
...
SActorCanvas::FScopedWidgetSlotArguments SActorCanvas::AddActorSlot(UIndicatorController* Indicator)
{
...
void SActorCanvas::GetOffsetAndSize(const UIndicatorController* Indicator,
	FVector2D& OutSize,
	FVector2D& OutOffset,
	...
	FVector2D& OutPaddingMax) const
{
	...
	switch (Indicator->GetHAlignment())
	{
	...
	switch (Indicator->GetVAlignment())
	{
	case VAlign_Top:
		OutOffset.Y = 0.0f;
